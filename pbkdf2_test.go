// 直接抄的官方库的单元测试,Which is从RFC 6070拿来的例子作为测试用例
...
func testHash(t *testing.T, h func() hash.Hash, hashName string, vectors []testVector) {
	...
	for i, v := range vectors {
		o := p.MakeKey([]byte(v.password), []byte(v.salt), v.iter, len(v.output))
...
func benchmark(b *testing.B, h func() hash.Hash) {
	...
	salt := make([]byte, 8)
	for i := 0; i < b.N; i++ {
		password = p.MakeKey(password, salt, 4096, len(password))
	}
	sink += password[0]
}
